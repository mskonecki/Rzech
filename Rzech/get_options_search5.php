...
$get['fuel_name'] = htmlentities($_GET['fuel_name']) ?? 'none';
...
try
{
    ...
    $sql = $connection->prepare('SELECT DISTINCT bodyTypeName
                                 FROM AdDetails LEFT JOIN BodyType ON (BodyType = bodyTypeID)
                                 WHERE EXISTS(SELECT detailsID
			                     FROM Ad LEFT JOIN BrandModel ON (brandmodelID = IDBrandModel) LEFT JOIN Fuel ON (fuel = fuelID)
			                     WHERE brand = :brand AND model = :model AND productionDate = :dateYear AND fuelName = :fuel
                                 AND detailsID = adDetailsId)');
                                
        $sql->bindParam(':brand',$get['brand_name']);
        $sql->bindParam(':model',$get['model_name']);
        $sql->bindParam(':dateYear',$get['year_value']);
        $sql->bindParam(':fuel',$get['fuel_name']);
        $sql->execute();
